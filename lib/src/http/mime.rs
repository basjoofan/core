...
fn from_ext(ext: &str) -> Option<&'static str> {
    ...
    lookup(MIME_TYPES, ext)
}

fn lookup(map: &'static [(&str, &str)], key: &str) -> Option<&'static str> {
    map.binary_search_by_key(&key.to_ascii_lowercase(), |(k, _)| k.to_string())
        .ok()
        .map(|i| map[i].1)
}
...
#[test]
fn test_mime_type() {
    assert_eq!(from_ext("gif"), Some("image/gif"));
    assert_eq!(from_ext("TXT"), Some("text/plain"));
    assert_eq!(from_ext("blahblah"), None);

    assert_eq!(from_path(Path::new("/path/to/file.gif")), Some("image/gif"));
    assert_eq!(from_path("/path/to/file.gif"), Some("image/gif"));
    assert_eq!(from_path("/path/to/file"), None);
}
